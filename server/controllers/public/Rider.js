...
router.post("/register", async (req, res) => {
  try {
    let { RidersName, email, phone, password,vehicleType,licenseNumber ,address } =req.body;

    // duplicate user 
    let duplicate = await riderModel.findOne({email})
    if(duplicate){
    ...
    let customer = {
        ...
        await riderModel.create(customer)
    // send email 
    let emailUrl = `http://localhost:8080/riders/email-verify/${emailToken}`;
    let phoneUrl = `http://localhost:8080/riders/phone-verify/${phoneToken}`;
...
router.post("/login",async(req,res)=>{
    try {
        ...
        let check = await riderModel.findOne({email})
        if(!check){
...
router.post("/forgetpass",async(req,res)=>{
  try {
    ...
    let one = await riderModel.findOne({email})
    if(!one){
      return res.status(401).json("The rider doesn't exist")
    }
